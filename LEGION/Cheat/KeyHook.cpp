...
LRESULT APIENTRY CKeyHook::hkWndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_MOUSEWHEEL:
	{
		if (GET_WHEEL_DELTA_WPARAM(wParam) < 0 && g_Config.g_Combat.bFastScroll && FindPlayerPed()->m_nActiveWeaponSlot != 0)
		{
				FindPlayerPed()->SetCurrentWeapon(+1);
		}
		break;
	}
	case WM_SYSKEYDOWN:
	...
	if (isKeyPressed(g_Config.g_Hotkeys.iActivate))
	{
