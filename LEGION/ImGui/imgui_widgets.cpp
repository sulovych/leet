...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImU32 border_col = IM_COL32(54, 54, 54, 255);
    window->DrawList->AddRect(check_bb.Min, check_bb.Max, border_col, style.FrameRounding, ImDrawCornerFlags_All, border_size);
    RenderFrame(check_bb.Min, check_bb.Max, GetColorU32((held && hovered) ? ImGuiCol_CheckBoxBg : hovered ? ImGuiCol_CheckBoxBg : ImGuiCol_CheckBoxBg), true, style.FrameRounding);
...
bool ImGui::Checkbox2(const char* label, bool* v)
{
    ...
    RenderNavHighlight(total_bb, id);
    const float border_size = 1.0f;
    const ImU32 border_col = IM_COL32(0, 0, 0, 255);
    window->DrawList->AddRect(check_bb.Min, check_bb.Max, border_col, 0.0f, ImDrawCornerFlags_All, border_size);
    RenderFrame(check_bb.Min, check_bb.Max, GetColorU32((held && hovered) ? IM_COL32(0, 0, 0, 0) : hovered ? IM_COL32(0, 0, 0, 0) : IM_COL32(0, 0, 0, 0)), true, style.FrameRounding);
    ImU32 check_col = IM_COL32(68, 230, 172, 120);
    bool mixed_value = (window->DC.ItemFlags & ImGuiItemFlags_MixedValue) != 0;
    if (mixed_value)
    {
        // Undocumented tristate/mixed/indeterminate checkbox (#2644)
    ...
    else if (*v)
    {
        ...
        window->DrawList->AddRectFilled(check_bb.Min + ImVec2(pad, pad), check_bb.Max - ImVec2(pad, pad), check_col, 0);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    // CTRL-clicking on Slider turns it into an input box
    const bool hovered = ItemHoverable(frame_bb, id);
    const bool temp_input_allowed = (flags & ImGuiSliderFlags_NoInput) == 0;
    bool temp_input_is_active = temp_input_allowed && TempInputIsActive(id);
    if (!temp_input_is_active)
    {
        ...
        if (clicked || g.NavActivateId == id || g.NavInputId == id)
        {
            ...
            FocusWindow(window);
            g.ActiveIdUsingNavDirMask |= (1 << ImGuiDir_Left) | (1 << ImGuiDir_Right);
            if (temp_input_allowed && ((clicked && g.IO.KeyCtrl) || g.NavInputId == id))
            {
               temp_input_is_active = true;
               FocusableItemUnregister(window);
            }
    ...
    if (temp_input_is_active)
    {
        ...
        bool value_changed = TempInputScalar(frame_bb, id, label, data_type, p_data, format, is_clamp_input ? p_min : NULL, is_clamp_input ? p_max : NULL);

        // Отображаем значение над слайдером
        char value_buf[64];
        const char* value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, p_data, format);
        ImVec2 text_size = ImGui::CalcTextSize(value_buf);
        ImVec2 text_pos = ImVec2(frame_bb.Max.x - text_size.x, frame_bb.Min.y - 8.0f); // Позиция текста над слайдером
        RenderTextClipped(text_pos, text_pos + text_size, value_buf, value_buf_end, NULL, ImVec2(1.0f, 0.5f)); // Основной текст, выравнивание по правому краю

        return value_changed;
    }

    // Draw frame
    const ImU32 line_col = GetColorU32(g.ActiveId == id ? ImGuiCol_FrameBgActive : g.HoveredId == id ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg);
    RenderNavHighlight(frame_bb, id);
    ImVec2 line_start = ImVec2(frame_bb.Min.x, frame_bb.Min.y + frame_bb.GetHeight() * 0.5f);
    ImVec2 line_end = ImVec2(frame_bb.Max.x, frame_bb.Min.y + frame_bb.GetHeight() * 0.5f);
    window->DrawList->AddLine(line_start, line_end, line_col, 6.0f); 
    ...
    if (grab_bb.Max.x > grab_bb.Min.x) {
       window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x, frame_bb.Min.y + frame_bb.GetHeight() * 0.3f), ImVec2(grab_bb.Max.x, frame_bb.Min.y + frame_bb.GetHeight() * 0.7f),GetColorU32(g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab),style.GrabRounding);
        
        ImVec2 circle_center = ImVec2(grab_bb.Max.x, (frame_bb.Min.y + frame_bb.Max.y) * 0.5f); 
        const float circle_radius = 5.2f; // Радиус кружка
        window->DrawList->AddCircleFilled(circle_center, circle_radius, IM_COL32(255, 255, 255, 255)); // Белый цвет
    }
    ...
    ImVec2 text_size = ImGui::CalcTextSize(value_buf);
    ImVec2 text_pos = ImVec2(frame_bb.Max.x - text_size.x, frame_bb.Min.y - 8.0f); // Привязка к правому краю

    RenderTextClipped(text_pos, text_pos + text_size, value_buf, value_buf_end, NULL, ImVec2(1.0f, 0.5f)); // Основной текст, выравнивание по правому краю

    // Новая позиция для лейбла, выравненная по левому краю
    if (label_size.x > 0.0f) {
        ImVec2 label_pos = ImVec2(frame_bb.Min.x, frame_bb.Min.y - 8.0f); // Привязка к левому краю
        RenderText(label_pos, label); // Рендерим лейбл по левому краю
    }
    return value_changed;
}
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (is_multiline) // Open group before calling GetID() because groups tracks id created within their scope,
        BeginGroup();
    const ImGuiID id = window->GetID(label);
    const ImVec2 label_size = ImVec2(0.0f, 12.0f);
    const ImVec2 frame_size = CalcItemSize(size_arg, CalcItemWidth(), (is_multiline ? g.FontSize * 8.0f : (label ? label_size.y : 0.0f)) + style.FramePadding.y * 2.0f);
