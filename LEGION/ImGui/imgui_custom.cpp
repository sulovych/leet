...
bool ImGui::TabButton(const char* label, int* tab, int index, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
	...
	if (*tab == index) {
		...
		// Полоска выбора
		ImVec2 start = ImVec2(bb.Min.x - 2, bb.Min.y + 1);
		ImVec2 end = ImVec2(bb.Min.x - 2, bb.Max.y - 1);
		draw_list->AddLine(start, end, IM_COL32(0, 0, 0, 255), 6.0f);
		draw_list->AddLine(start, end, IM_COL32(68, 230, 172, 255), 2.0f);
	}

	else col = GetColorU32(hovered ? ImGuiCol_TabHovered : ImGuiCol_Tab);
	RenderNavHighlight(bb, id);
	RenderFrame(bb.Min, bb.Max, col, true, 0.0);
